function [ EssMatGuess, numb_it ] = helmke_alg_smoothed( InGuess, Ess_prev, Ess_prev_prev, acc, m_1, m_2, max_it )
...
[U_prev, ~, V_prev] = svd(Ess_prev);
[U_prev_prev, ~, V_prev_prev] = svd(Ess_prev_prev);
...
U_est = U_prev*expm([0 -x_est(3)/sqrt(2) x_est(2); x_est(3)/sqrt(2) 0 -x_est(1); -x_est(2) x_est(1) 0]);
V_est = V_prev*expm([0 x_est(3)/sqrt(2) x_est(5); -x_est(3)/sqrt(2) 0 -x_est(4); -x_est(5) x_est(4) 0]);
Ess_est_im = U_est * E_0 * V_est';
Ess_est = real(Ess_est_im);
...
for i=1:max_it
    ...
    D_D_vec = kron(V_k', U_k')* (E(:)-Ess_est(:));
    D_D = reshape(D_D_vec, [3,3]);
    H_D_til = [Q_1', Q_2'] * [-(kron(D_D*E_0, eye(3))), kron(D_D, E_0); kron(D_D', E_0), -kron(E_0*D_D,eye(3))] * [Q_1; Q_2];
